...
function main(argv, stdout, stderr, exit) {
  ...
  program.args.forEach(function(file) {
    ...
    log.items(info.provided.map(function(item) {
        return item  + (_.contains(info.suppressedProvide, item) ? ' (ignored)' : '');
    }));
    log.info('');
    log.info('Required:');
    log.items(info.required.map(function(item) {
        return item  + (_.contains(info.suppressedRequire, item) ? ' (ignored)' : '');
    }));
    log.info('');
    ...
    unnecessaryProvide = _.difference(unnecessaryProvide, info.suppressedProvide);
    ...
    unnecessaryRequire = _.difference(unnecessaryRequire, info.suppressedRequire);
