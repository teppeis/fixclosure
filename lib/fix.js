...
function getProvideRequireSrc(buf, info) {
  ...
  var suppressedRequire = info.suppressedRequire.map(function(namespace) {
    /*jshint quotmark:false */
    return "goog.require('" + namespace + "'); // fixclosure: ignore";
  });

  if (info.toProvide.length > 0 && toRequire.length + suppressedRequire.length > 0) {
    buf.push('');
  }

  toRequire.concat(suppressedRequire).sort().forEach(function(req) {
    buf.push(req);
  });
}
