...
Parser.prototype.parse = function(src) {
  ...
  var toRequire = this.extractToRequire_(parsed);
 ...
 * @private
 */
Parser.prototype.extractToRequire_ = function(parsed) {
  var assigned = this.extractAssigned_(parsed);
  var toRequire = parsed.
    ...
    sort().
    reduce(this.uniq_, []);
  return _.difference(toRequire, assigned);
};
 ...
 * @private
 */
Parser.prototype.extractAssigned_ = function(parsed) {
  return parsed.
    map(this.assignedMapper_.bind(this)).
    filter(this.isDefAndNotNull_).
    sort().
    reduce(this.uniq_, []);
};
 ...
 * @private
 */
Parser.prototype.assignedMapper_ = function(use) {
  var name = use.name.join('.');
  switch (use.node.type) {
    case Syntax.AssignmentExpression:
      if (use.key === 'left') {
