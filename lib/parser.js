...
const Parser = function(opt_options) {
  ...
  this.requireRoots_ = Object.assign(def.getRoots(), this.provideRoots_);
  ...
  if (options.replaceMap) {
    Object.assign(this.replaceMap_, options.replaceMap);
  }
...
Parser.prototype.extractToRequireFromJsDoc_ = function(comments) {
  return comments
    .filter(comment => {
      // JSDoc Style
      return comment.type === 'CommentBlock' && /^\*/.test(comment.value);
    })
    .reduce((prev, comment) => {
      const jsdoc = doctrine.parse(`/* ${comment.value}*/`, {unwrap: true});
