...
Parser.prototype.extractToProvide_ = function(parsed, comments) {
  ...
  var result = parsed.
    ...
    sort().
    reduce(this.uniq_, []);
  return this.removeDuplicated_(result);
};
...
/**
 * @param {string} name
 * @return {boolean} true iff given name is CONSTANT in namespace
 */
Parser.prototype.isNamespaceConst_ = function(name) {
  var split = name.split('.');
  var memberName = split.pop();
  var upperLevel = split.join('.');
  return /^[A-Z_]+$/.test(memberName) && /^[a-z.]*$/.test(upperLevel);
};
...
/**
 * @param {string} name
 * @return {string} upper level of given name. e.g. 'goog.ui' for 'goog.ui.Component'
 */
Parser.prototype.upperLevelOf_ = function(name) {
  var split = name.split('.');
  split.pop();
  return split.join('.');
};
...
/**
 * @param {string} name
 * @return {boolean} true iff given value is name of namespace
 */
Parser.prototype.isNamespace_ = function(name) {
  return _.reduce(name.split('.'), function(memo, part) {
    return memo && part.toLowerCase() === part;
  }, true);
};

/**
 * @param {string} string
 * @return {boolean} true iff given value upper camel case
 */
Parser.prototype.isUpperCamelCase_ = function(string) {
  var result = (string.indexOf('_') === -1) &&
    (string.indexOf('-') === -1) &&
...
/**
 * @param {string} name
 * @return {boolean} true iff given value is name of Class/Enum/TypeDef
 */
Parser.prototype.isClassLike_ = function(name) {
  var split = name.split('.');
  var className = split.pop();
  var upperLevel = split.join('.');
  return (this.isNamespace_(upperLevel) || this.isClassLike_(upperLevel)) && this.isUpperCamelCase_(className);
};

/**
 * @param {!Array.<string>} provided actual provided namespace/class/others
 * @return {!Array.<string>} expected provided namespace/class/others which has no duplicated one
 ...
 */
Parser.prototype.removeDuplicated_ = function(provided) {
  return _.filter(provided, function(name) {
    var upperLevel = name;
    while (! this.isNamespace_(this.upperLevelOf_(upperLevel))) {
      upperLevel = this.upperLevelOf_(upperLevel);
    }

    if (upperLevel === name) {
      return true;
    } else {
      // remove if upper level class exists in actual goog.provide()
      return ! (this.isClassLike_(upperLevel) && _.contains(provided, upperLevel));
    }
  }, this);
};
