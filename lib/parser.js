...
Parser.prototype.parse = function(src) {
  ...
  const parsed = this.parseAst_(ast);
  const provided = this.extractProvided_(parsed);
  const required = this.extractRequired_(parsed);
  const ignored = this.extractSuppressUnused_(parsed, ast.comments);
  const toProvide = this.extractToProvide_(parsed, ast.comments);
  const toRequireFromJsDoc = this.extractToRequireFromJsDoc_(ast.comments);
  const toRequire = this.extractToRequire_(parsed, toProvide, ast.comments, toRequireFromJsDoc);
...
Parser.prototype.getSuppressProvideComments_ = function(comments) {
  return comments.filter(comment => {
    return comment.type === 'Line' && /^\s*fixclosure\s*:\s*suppressProvide\b/.test(comment.value);
  });
};
...
Parser.prototype.getSuppressRequireComments_ = function(comments) {
  return comments.filter(comment => {
    return comment.type === 'Line' && /^\s*fixclosure\s*:\s*suppressRequire\b/.test(comment.value);
  });
};
...
Parser.prototype.suppressFilter_ = function(comments, use) {
  ...
  const suppressComment = comments.some(comment => {
