...
let Syntax = require('estraverse-fb').Syntax;
  ...
  switch (node.type) {
    ...
    case Syntax.JSXMemberExpression:
      if (hasObjectIdentifier_(node) && !hasScope_(node, this.parents())) {
        let parents = this.parents().concat(node).reverse();
        let path = this.path().concat('object').reverse();
        let use = registerIdentifier_(node.object, parents, path);
        if (use) {
          uses.push(use);
        }
      }
